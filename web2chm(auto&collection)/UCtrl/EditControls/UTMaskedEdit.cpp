...
// UTMaskedEdit.cpp : implementation file
//
/////////////////////////////////////////////////////////////////////////////

#include "stdafx.h"
#include "UTMaskedEdit.h"    // CUTMaskedEdit
...
IMPLEMENT_DYNAMIC(CUTMaskedEdit, CEdit)

CUTMaskedEdit::CUTMaskedEdit(LPCTSTR pszMask/*=_T("")*/) : 
	m_bInsertMode(TRUE),
...
CUTMaskedEdit::~CUTMaskedEdit()
{
	DeleteContents();
}


BEGIN_MESSAGE_MAP(CUTMaskedEdit, CEdit)
	//{{AFX_MSG_MAP(CUTMaskedEdit)
	ON_WM_KEYDOWN()
	ON_WM_CHAR()
	ON_WM_SETFOCUS()
...
/////////////////////////////////////////////////////////////////////////////
// CUTMaskedEdit operations

void CUTMaskedEdit::DeleteContents()
{
...
CString CUTMaskedEdit::GetMask() const
{
	CString csMask;
	CMaskData* pobjData=NULL;
	for(POSITION pos=m_listData.GetHeadPosition(); pos;)
...
void CUTMaskedEdit::SetMask(LPCTSTR pszMask)
{
	if(pszMask==NULL)
	{
		pszMask=_T("");
...
CString CUTMaskedEdit::GetInputData() const
{
	CString csInputData;
...
CString CUTMaskedEdit::GetInputData(LPCTSTR lpszText) const
{
...
BOOL CUTMaskedEdit::SetInputData(LPCTSTR pszInputData, int nBeginPos/*=0*/, 
...
TCHAR CUTMaskedEdit::GetPromptSymbol()
{
	return m_chPromptSymbol;
}

void CUTMaskedEdit::SetPromptSymbol(TCHAR chNewPromptSymbol)
{
...
void CUTMaskedEdit::EmptyData(BOOL bOnlyInput/*=FALSE*/)
{
	if(m_listData.GetCount()==0)
	{
		DeleteContents();
...
BOOL CUTMaskedEdit::IsInputEmpty()
{
	if(m_listData.GetCount()==0)
	{
		CString csInputData;
...
BOOL CUTMaskedEdit::GetInsertMode() const
{
...
void CUTMaskedEdit::SetInsertMode(BOOL bInsertMode)
{
	// The standard CEdit control does not support over-typing. 
	// This flag is used to manage over-typing internally. 
...
BOOL CUTMaskedEdit::GetAutoTab() const
{
...
void CUTMaskedEdit::SetAutoTab(BOOL bAutoTab)
{
	// The standard CEdit control does not support AutoTab mode. 
	// This flag is used to manage AutoTab mode internally. 
	m_bAutoTab=bAutoTab;
...
CString CUTMaskedEdit::ShowMask() const
{
	CString csShow;
	CMaskData* pobjData=NULL;
	for(POSITION pos=m_listData.GetHeadPosition(); pos;)
...
BOOL CUTMaskedEdit::IsInputData(int nPosition) const
{
	if(m_listData.GetCount()==0)
	{
		return TRUE;
...
int CUTMaskedEdit::DeleteRange(int nSelectionStart, int nSelectionEnd)
{
...
int CUTMaskedEdit::InsertAt(int nSelectionStart, TCHAR chNewChar)
{
...
int CUTMaskedEdit::SetAt(int nSelectionStart, TCHAR chNewChar)
{
...
int CUTMaskedEdit::GetNextInputLocation(int nSelectionStart)
{
...
int CUTMaskedEdit::GetPreviousInputLocation(int nSelectionStart)
{
	// One of the functions of this edit control is that it skips over literals. 
	// We need a function to help skip to the next position. 
	int nNextInputLocation=nSelectionStart;
...
int CUTMaskedEdit::GetEmptyInputLocation(int nSelectionStart)
{
...
void CUTMaskedEdit::Update(int nSelectionStart/*=0*/)
{
...
void CUTMaskedEdit::UpdateInsertionPointForward(int nSelectionStart)
{
...
void CUTMaskedEdit::UpdateInsertionPointBackward(int nSelectionStart)
{
	int nNewInsertionPoint=GetPreviousInputLocation(nSelectionStart);
	SetSel(nNewInsertionPoint, nNewInsertionPoint);
}

void CUTMaskedEdit::ValidationError()
{
	::MessageBeep(MB_ICONEXCLAMATION);
}

/////////////////////////////////////////////////////////////////////////////
// CUTMaskedEdit overrides

void CUTMaskedEdit::PreSubclassWindow() 
{
...
/////////////////////////////////////////////////////////////////////////////
// CUTMaskedEdit message handlers

BOOL CUTMaskedEdit::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
{
...
void CUTMaskedEdit::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
...
void CUTMaskedEdit::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	// If there is no mask, then exit quickly performing the default operation. 
	if(m_listData.GetCount()==0)
	{
...
void CUTMaskedEdit::OnSetFocus(CWnd* pOldWnd) 
{

	//Ultimate Grid Code - setup the mask 
	LPCTSTR m = m_ctrl->m_editCell.GetMask();
	SetMask(m);
	SetWindowText(m_ctrl->m_editCell.GetText());
...
LONG CUTMaskedEdit::OnCut(UINT wParam, LONG lParam)
{
...
LONG CUTMaskedEdit::OnCopy(UINT wParam, LONG lParam)
{
	UNREFERENCED_PARAMETER(wParam);
	UNREFERENCED_PARAMETER(lParam);
...
LONG CUTMaskedEdit::OnPaste(UINT wParam, LONG lParam) 
{
	UNREFERENCED_PARAMETER(wParam);
	UNREFERENCED_PARAMETER(lParam);
...
LONG CUTMaskedEdit::OnClear(UINT wParam, LONG lParam)
{
	UNREFERENCED_PARAMETER(wParam);
	UNREFERENCED_PARAMETER(lParam);
...
void CUTMaskedEdit::OnKillfocus() 
{
...
BOOL CUTMaskedEdit::OnValidate() 
{
	// by default return TRUE
	// one can overwrite this function to provide validation capability
	// in CUTMaskedEdit derived class
	return TRUE;
}

void CUTMaskedEdit::OnLButtonDown(UINT nFlags, CPoint point)
{
...
LONG CUTMaskedEdit::OnSetText(UINT wParam, LONG lParam)
{
	UNREFERENCED_PARAMETER(wParam);
	UNREFERENCED_PARAMETER(lParam);
...
int CUTMaskedEdit::RPtoLP(int nRealPos) const 
{
	// All CUTMaskedEdit functions that take cusor position as argument interpret it
...
int CUTMaskedEdit::LPtoRP(int nLogicalPos) const 
{
	// All CUTMaskedEdit functions that take cusor position as argument interpret it
	// as real position within edit control (taking into account all symbols including 
...
BOOL CUTMaskedEdit::NotifyParent(UINT nNotificationID)
{
	CWnd* pParentWnd=GetParent();
	if(pParentWnd==NULL)
		return FALSE;
