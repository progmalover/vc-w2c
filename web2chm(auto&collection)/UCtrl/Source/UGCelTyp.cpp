...
void CUGCellType::DrawBackground(CDC *dc,RECT *rect,COLORREF backcolor, int row, int col, CUGCell * cell, bool current, bool selected)
{
	...
	if (cell != NULL)
	{
		...
		if (m_useThemes)
		{
			...
			isOwnerDraw = FALSE;
			UGXPCellType xpStyle =  cell->GetXPStyle();
			if(xpStyle != XPCellTypeTopCol)
			  isOwnerDraw = UGXPThemes::DrawBackground(NULL, *dc, xpStyle, state, &rcTheme, NULL);
			else
			{
				isOwnerDraw = TRUE;
				CDrawingManager  dm(*dc);
				dm.FillGradient(&rcTheme,afxGlobalData.clrBarDkShadow,afxGlobalData.clrBarHilite);
			}
			//dc->FillRect(&rcTheme,&afxGlobalData.brInactiveCaption);
